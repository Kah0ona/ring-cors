...
(deftest test-dynamic-allow-origin
  (testing "Testing an allow-origin with a callback function for dynamic checks, where it returns true (allowed)"
    (let [response ((wrap-cors
                     (fn [_] {})
                     :access-control-allow-origin (fn my-callback
                                                    [req]
                                                    true)
                     :access-control-allow-methods [:get :post :patch :put :delete])
                    {:request-method :get
      ...
      (is (= {:headers
             ...
             response))))
  (testing "Testing an allow-origin with a callback function for dynamic checks, where it returns false (not allowed)"
    (let [response ((wrap-cors
                     (fn [_] {})
                     :access-control-allow-origin (fn my-callback
                                                    [req]
